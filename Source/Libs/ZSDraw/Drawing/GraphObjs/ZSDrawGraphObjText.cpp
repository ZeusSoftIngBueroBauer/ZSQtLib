    ...
    m_pTrcAdminObjPaint(nullptr)
{
    m_pTrcAdminObjCtorsAndDtor = CTrcServer::GetTraceAdminObj(
        NameSpace() + "::Drawing::GraphObjs", ClassName() + "::CtorsAndDtor");
    m_pTrcAdminObjItemChange = CTrcServer::GetTraceAdminObj(
        NameSpace() + "::Drawing::GraphObjs", ClassName() + "::ItemChange");
    m_pTrcAdminObjBoundingRect = CTrcServer::GetTraceAdminObj(
        NameSpace() + "::Drawing::GraphObjs", ClassName() + "::BoundingRect");
    m_pTrcAdminObjPaint = CTrcServer::GetTraceAdminObj(
        NameSpace() + "::Drawing::GraphObjs", ClassName() + "::Paint");
...
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
    if (areMethodCallsActive(m_pTrcAdminObjItemChange, EMethodTraceDetailLevel::ArgsNormal)) {
        strMthInArgs = qTextCursor2Str(i_cursor);
    }
    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObjItemChange,
        /* iDetailLevel */ EMethodTraceDetailLevel::EnterLeave,
...
//------------------------------------------------------------------------------
{
    ...
    if (m_editMode != i_eMode) {
        ...
        if (m_editMode == EEditMode::None) {
            ...
            QGraphicsTextItem_setTextInteractionFlags(Qt::NoTextInteraction);
        }
...
/*! @brief Reimplements the virtual method of base class CGraphObj.
*/
QRectF CGraphObjText::getEffectiveBoundingRectOnScene() const
//------------------------------------------------------------------------------
{
    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObjBoundingRect,
...
//------------------------------------------------------------------------------
{
    ...
    if (i_change == ItemSceneHasChanged) {
        ...
        if (m_pDrawingScene->getMode() == EMode::Edit && isSelected()) {
            ...
            else /*if (m_editMode == EEditMode::None || m_editMode == EEditMode::ModifyingContent)*/ {
                hideSelectionPoints();
            }
        }
        else {
