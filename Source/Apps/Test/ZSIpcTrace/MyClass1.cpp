...
//------------------------------------------------------------------------------
{
    ...
    QString strResult = "Hello World";
    if (mthTracer.areMethodCallsActive(EMethodTraceDetailLevel::ArgsNormal)) {
        mthTracer.setMethodReturn(strResult);
    }
    return strResult;
}

//------------------------------------------------------------------------------
CTrcAdminObj* CMyClass1::getTrcAdminObj()
//------------------------------------------------------------------------------
{
    return s_trcAdminObjRefAnchor.trcAdminObj();
}
...
//------------------------------------------------------------------------------
{
    ...
    if (mthTracer.areMethodCallsActive(EMethodTraceDetailLevel::ArgsNormal)) {
        QString strMthRet = QString(m_pMyClass2 == nullptr ? "null" : m_pMyClass2->objectName());
...
//------------------------------------------------------------------------------
QString CMyClass1::instMethod1(const QString& i_strMthInArgs)
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
    if (areMethodCallsActive(s_trcAdminObjRefAnchor.trcAdminObj(), EMethodTraceDetailLevel::ArgsNormal)) {
        strMthInArgs = i_strMthInArgs;
    }
    CMethodTracer mthTracer(
        /* pAdminObj    */ s_trcAdminObjRefAnchor.trcAdminObj(),
...
//------------------------------------------------------------------------------
QString CMyClass1::instMethod2(const QString& i_strMthInArgs)
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
    if (areMethodCallsActive(s_trcAdminObjRefAnchor.trcAdminObj(), EMethodTraceDetailLevel::ArgsNormal)) {
...
//------------------------------------------------------------------------------
QString CMyClass1::instMethod3(const QString& i_strMthInArgs)
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
    if (areMethodCallsActive(s_trcAdminObjRefAnchor.trcAdminObj(), EMethodTraceDetailLevel::ArgsNormal)) {
...
//------------------------------------------------------------------------------
{
    ...
    if (mthTracer.isRuntimeInfoActive(ELogDetailLevel::DebugVerbose)) {
        ...
        while ((fCurrTime_s - fStartTime_s) < fDuration_s) {
            QString strAddInfo = "Im sending data now for " + QString::number(fCurrTime_s - fStartTime_s, 'f', 3) + " seconds. " +
                "I will send for another " + QString::number(fDuration_s - (fCurrTime_s - fStartTime_s)) + " seconds.";
