...
ifeq ($(PLATFORM), Win32)
    ifeq ($(CONFIG), Release)
        ...
        ZSQTLIBS := $(ZSQTLIBS) $(LIB_PREFIX)ZSRemCmdGUI.$(LIB_EXT)
    else ifeq ($(CONFIG), Debug)
        ...
        ZSQTLIBS := $(ZSQTLIBS) $(LIB_PREFIX)ZSRemCmdGUId.$(LIB_EXT)
    endif
else ifeq ($(PLATFORM), Linux)
    # On linux machines the Qt version is resolved using symbolic links and
    # the version is encode in the file extensions (e.g. libQtCore.so.4.8.7).
    ...
    ifeq ($(CONFIG), Release)
        ...
        ZSQTLIBS := $(ZSQTLIBS) $(ZSQTLIB_DIR_LIB)/$(LIB_PREFIX)ZSRemCmdGUI.$(LIB_EXT)
    else ifeq ($(CONFIG), Debug)
        ...
        ZSQTLIBS := $(ZSQTLIBS) $(ZSQTLIB_DIR_LIB)/$(LIB_PREFIX)ZSRemCmdGUId.$(LIB_EXT)
    endif
endif
export ZSQTLIBS
...
help:
	...
	@echo .
	@echo "Aufruf   : make -f ZSRemCmdGUIBinding4Python.mk [CONFIG=<Debug|Release>] [Target]"
	@echo .
	@echo "Targets  : build   - (default) Creates the target."
	@echo "           clean   - Deletes all output files."
	@echo "           help    - Shows this help text."
	@echo .
	@echo "Example  : make -f ZSRemCmdGUIBinding4Python.mk CONFIG=Debug build
	@echo .
