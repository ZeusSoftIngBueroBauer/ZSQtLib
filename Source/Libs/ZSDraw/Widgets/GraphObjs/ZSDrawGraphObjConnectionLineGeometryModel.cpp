...
/*******************************************************************************
class ZSDRAWDLL_API CModelGraphObjConnectionLineGeometry : publicQAbstractTableModel
*******************************************************************************/
...
//------------------------------------------------------------------------------
QString CModelGraphObjConnectionLineGeometry::column2Str(int i_clm)
//------------------------------------------------------------------------------
{
...
//------------------------------------------------------------------------------
QString CModelGraphObjConnectionLineGeometry::itemDataRole2Str(int i_iRole)
//------------------------------------------------------------------------------
{
...
//------------------------------------------------------------------------------
QString CModelGraphObjConnectionLineGeometry::modelIndex2Str(const QModelIndex& i_modelIdx) const
//------------------------------------------------------------------------------
{
    QString str;
...
*/
CModelGraphObjConnectionLineGeometry::SLabelSettings CModelGraphObjConnectionLineGeometry::SLabelSettings::fromGraphObj(
...
//------------------------------------------------------------------------------
CModelGraphObjConnectionLineGeometry::SLabelSettings::SLabelSettings() :
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
bool CModelGraphObjConnectionLineGeometry::SLabelSettings::operator == (const SLabelSettings& i_other) const
//------------------------------------------------------------------------------
{
    bool bEqual = true;
...
/*! @brief Model for the geometry values of a line.
    ...
    @param [in] i_pObjParent
        Parent object.
*/
CModelGraphObjConnectionLineGeometry::CModelGraphObjConnectionLineGeometry(
    ...
    m_pTrcAdminObjNoisyMethods(nullptr)
{
    ...
    QObject::connect(
        ...
        this, &CModelGraphObjConnectionLineGeometry::onDrawingSceneDrawingSizeChanged);
...
*/
void CModelGraphObjConnectionLineGeometry::setFont(const QFont& i_font)
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
    if (areMethodCallsActive(m_pTrcAdminObj, EMethodTraceDetailLevel::ArgsNormal)) {
...
*/
QFont CModelGraphObjConnectionLineGeometry::font() const
//------------------------------------------------------------------------------
{
    return m_font;
}
...
*/
bool CModelGraphObjConnectionLineGeometry::setKeyInTree(const QString& i_strKeyInTree)
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
    if (areMethodCallsActive(m_pTrcAdminObj, EMethodTraceDetailLevel::ArgsNormal)) {
    ...
    if (m_strKeyInTree != i_strKeyInTree) {
        ...
        if (m_pGraphObjConnectionLine != nullptr) {
            QObject::disconnect(
                m_pGraphObjConnectionLine, &CGraphObj::geometryOnSceneChanged,
                this, &CModelGraphObjConnectionLineGeometry::onGraphObjGeometryOnSceneChanged);
            //QObject::disconnect(
            //    m_pGraphObjConnectionLine, &CGraphObj::geometryValuesUnitChanged,
            //    this, &CModelGraphObjConnectionLineGeometry::onGraphObjGeometryValuesUnitChanged);
            QObject::disconnect(
                m_pGraphObjConnectionLine, &CGraphObj::geometryLabelChanged,
                this, &CModelGraphObjConnectionLineGeometry::onGraphObjGeometryLabelChanged);
            QObject::disconnect(
                m_pGraphObjConnectionLine, &CGraphObj::aboutToBeDestroyed,
                this, &CModelGraphObjConnectionLineGeometry::onGraphObjAboutToBeDestroyed);
        }
        ...
        if (m_strKeyInTree.isEmpty()) {
            ...
            m_pGraphObjConnectionLine = dynamic_cast<CGraphObjConnectionLine*>(m_pDrawingScene->findGraphObj(i_strKeyInTree));
        }
...
/*! @brief Returns the unique key of the graphical object currently shown in
           the widget.
*/
QString CModelGraphObjConnectionLineGeometry::getKeyInTree() const
//------------------------------------------------------------------------------
{
...
*/
bool CModelGraphObjConnectionLineGeometry::hasErrors() const
//------------------------------------------------------------------------------
{
    CMethodTracer mthTracer(
...
/*! @brief Checks whether the user has changed settings which haven't been
    ...
    @return true, if at least one data row has a different setting than the
            ...
            graphical object.
*/
bool CModelGraphObjConnectionLineGeometry::hasChanges() const
//------------------------------------------------------------------------------
{
    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObj,
    ...
    if (m_pGraphObjConnectionLine != nullptr) {
        ...
        CPhysValPolygon physValPolygon = m_pGraphObjConnectionLine->getPolygon(drawingSize.unit());
        bHasChanges = (m_physValPolygon != physValPolygon);
        if (!bHasChanges) {
            QList<SLabelSettings> arLabelSettings = getLabelSettings(m_pGraphObjConnectionLine);
...
*/
void CModelGraphObjConnectionLineGeometry::acceptChanges()
//------------------------------------------------------------------------------
{
    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObj,
    ...
    if (m_pGraphObjConnectionLine != nullptr && !hasErrors())
    {
        {   CRefCountGuard refCountGuard(&m_iContentChangedSignalBlockedCounter);

            if (m_pGraphObjConnectionLine != nullptr) {
                m_pGraphObjConnectionLine->setPolygon(m_physValPolygon);
                for (const SLabelSettings& labelSettings : m_arLabelSettings) {
                    labelSettings.m_bVisible ?
                        m_pGraphObjConnectionLine->showGeometryLabel(labelSettings.m_strValueName) :
                        m_pGraphObjConnectionLine->hideGeometryLabel(labelSettings.m_strValueName);
                    labelSettings.m_bLineVisible ?
                        m_pGraphObjConnectionLine->showGeometryLabelAnchorLine(labelSettings.m_strValueName) :
                        m_pGraphObjConnectionLine->hideGeometryLabelAnchorLine(labelSettings.m_strValueName);
                }
...
*/
void CModelGraphObjConnectionLineGeometry::rejectChanges()
//------------------------------------------------------------------------------
{
    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObj,
...
*/
int CModelGraphObjConnectionLineGeometry::getLabelRowIndex(const QString& i_strLabelName) const
//------------------------------------------------------------------------------
{
    int iRowIdx = -1;
    for (const SLabelSettings& labelSettings : m_arLabelSettings) {
...
*/
QStringList CModelGraphObjConnectionLineGeometry::labelNames() const
//------------------------------------------------------------------------------
{
...
/*! @brief Returns a list with the current names of the labels.
*/
QString CModelGraphObjConnectionLineGeometry::getLabelName(int i_iRow) const
//------------------------------------------------------------------------------
{
...
*/
bool CModelGraphObjConnectionLineGeometry::isPointRow(int i_iRow) const
//------------------------------------------------------------------------------
{
...
//------------------------------------------------------------------------------
void CModelGraphObjConnectionLineGeometry::removePoint(const QString& i_strLabelName)
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
    if (areMethodCallsActive(m_pTrcAdminObj, EMethodTraceDetailLevel::ArgsNormal)) {
...
//------------------------------------------------------------------------------
void CModelGraphObjConnectionLineGeometry::insertPointBefore(const QString& i_strLabelName)
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
    if (areMethodCallsActive(m_pTrcAdminObj, EMethodTraceDetailLevel::ArgsNormal)) {
...
//------------------------------------------------------------------------------
void CModelGraphObjConnectionLineGeometry::insertPointAfter(const QString& i_strLabelName)
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
    if (areMethodCallsActive(m_pTrcAdminObj, EMethodTraceDetailLevel::ArgsNormal)) {
...
//------------------------------------------------------------------------------
int CModelGraphObjConnectionLineGeometry::rowCount(const QModelIndex& i_modelIdxParent) const
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
...
//------------------------------------------------------------------------------
int CModelGraphObjConnectionLineGeometry::columnCount(const QModelIndex& i_modelIdxParent) const
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
    if (areMethodCallsActive(m_pTrcAdminObjNoisyMethods, EMethodTraceDetailLevel::ArgsNormal)) {
...
//------------------------------------------------------------------------------
QVariant CModelGraphObjConnectionLineGeometry::data(const QModelIndex& i_modelIdx, int i_iRole) const
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
    if (areMethodCallsActive(m_pTrcAdminObjNoisyMethods, EMethodTraceDetailLevel::ArgsNormal)) {
    ...
    if (m_pGraphObjConnectionLine != nullptr && i_modelIdx.isValid()) {
            ...
            switch (iClm) {
                ...
                case EColumnXVal: {
                    if (i_iRole == Qt::DisplayRole) {
                        if (labelSettings.m_strValueName == CGraphObjConnectionLine::c_strGeometryLabelNameCenter) {
                            varData = m_physValPolygon.center().x().toString();
                        }
                        else if (labelSettings.m_strValueName == CGraphObjConnectionLine::c_strGeometryLabelNameWidth) {
                            varData = m_physValPolygon.width().toString();
                        }
                        else if (labelSettings.m_strValueName == CGraphObjConnectionLine::c_strGeometryLabelNameHeight) {
                            varData = m_physValPolygon.height().toString();
                        }
                        else if (labelSettings.m_strValueName == CGraphObjConnectionLine::c_strGeometryLabelNameAngle) {
                            varData = m_physValPolygon.angle().toString();
                        }
                        else if (labelSettings.m_strValueName.startsWith(CGraphObjConnectionLine::c_strGeometryLabelNameP)) {
                        ...
                        if (labelSettings.m_strValueName == CGraphObjConnectionLine::c_strGeometryLabelNameAngle) {
                            varData = 0.0;
                        }
                        else if (labelSettings.m_strValueName == CGraphObjConnectionLine::c_strGeometryLabelNameWidth) {
                        ...
                        if (labelSettings.m_strValueName == CGraphObjConnectionLine::c_strGeometryLabelNameHeight) {
                            if (m_eDimensionUnit == EScaleDimensionUnit::Metric) {
...
//------------------------------------------------------------------------------
bool CModelGraphObjConnectionLineGeometry::setData(
...
//------------------------------------------------------------------------------
QVariant CModelGraphObjConnectionLineGeometry::headerData(int i_iSection, Qt::Orientation i_orientation, int i_iRole) const
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
    if (areMethodCallsActive(m_pTrcAdminObjNoisyMethods, EMethodTraceDetailLevel::ArgsNormal)) {
...
//------------------------------------------------------------------------------
Qt::ItemFlags CModelGraphObjConnectionLineGeometry::flags(const QModelIndex& i_modelIdx) const
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
    if (areMethodCallsActive(m_pTrcAdminObjNoisyMethods, EMethodTraceDetailLevel::ArgsNormal)) {
...
*/
void CModelGraphObjConnectionLineGeometry::onDrawingSceneDrawingSizeChanged(const CDrawingSize& i_drawingSize)
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
    if (areMethodCallsActive(m_pTrcAdminObj, EMethodTraceDetailLevel::ArgsNormal)) {
...
////------------------------------------------------------------------------------
//void CModelGraphObjConnectionLineGeometry::onGraphObjGeometryValuesUnitChanged(CGraphObj* i_pGraphObj)
////------------------------------------------------------------------------------
//{
//    QString strMthInArgs;
//    if (areMethodCallsActive(m_pTrcAdminObj, EMethodTraceDetailLevel::ArgsNormal)) {
...
//                    CPhysValPolygon physValPolygon = m_pGraphObjConnectionLine->getPolygon(Units.Length.px);
...
//                }
//                else {
//                    CPhysValPolygon physValPolygon = m_pGraphObjConnectionLine->getPolygon();
//                    if (physValPolygon != m_physValPolygon) {
//                        bContentChanged = true;
//                        m_physValPolygon = physValPolygon;
//                    }
...
//------------------------------------------------------------------------------
void CModelGraphObjConnectionLineGeometry::onGraphObjGeometryOnSceneChanged(CGraphObj* i_pGraphObj)
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
    if (areMethodCallsActive(m_pTrcAdminObj, EMethodTraceDetailLevel::ArgsNormal)) {
        strMthInArgs = i_pGraphObj->keyInTree();
        if (m_pGraphObjConnectionLine == i_pGraphObj) {
            strMthInArgs += " [" + QString::number(m_pGraphObjConnectionLine->count()) + "]";
        }
    }
    CMethodTracer mthTracer(
        /* pTrcAdminObj       */ m_pTrcAdminObj,
    ...
    if (m_pGraphObjConnectionLine != i_pGraphObj) {
        throw CException(__FILE__, __LINE__, EResultInternalProgramError, "i_pGraphObj != m_pGraphObjConnectionLine");
    }
            ...
            if (m_pGraphObjConnectionLine != nullptr) {
                ...
                if (bContentChanged) {
                    if (bRowsInserted || bRowsRemoved) {
                        _beginResetModel();
                        m_arLabelSettings = getLabelSettings(m_pGraphObjConnectionLine);
                        _endResetModel();
                    }
                    else {
                        QModelIndex modelIdxTL = index(0, EColumnXVal);
...
//------------------------------------------------------------------------------
void CModelGraphObjConnectionLineGeometry::onGraphObjGeometryLabelChanged(
...
//------------------------------------------------------------------------------
{
    ...
    if (m_iContentChangedSignalBlockedCounter == 0)
    {
        {   CRefCountGuard refCountGuard(&m_iContentChangedSignalBlockedCounter);
            ...
            if (iRow >= 0 && iRow < m_arLabelSettings.size()) {
                SLabelSettings labelSettings = SLabelSettings::fromGraphObj(m_pGraphObjConnectionLine, i_strName, iRow);
                if (m_arLabelSettings[iRow] != labelSettings) {
                    m_arLabelSettings[iRow] = labelSettings;
...
//------------------------------------------------------------------------------
void CModelGraphObjConnectionLineGeometry::onGraphObjAboutToBeDestroyed(CGraphObj*)
//------------------------------------------------------------------------------
{
    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObj,
...
//------------------------------------------------------------------------------
void CModelGraphObjConnectionLineGeometry::clearModel()
//------------------------------------------------------------------------------
{
    CMethodTracer mthTracer(
        /* pTrcAdminObj       */ m_pTrcAdminObj,
...
//------------------------------------------------------------------------------
void CModelGraphObjConnectionLineGeometry::fillModel()
//------------------------------------------------------------------------------
{
    CMethodTracer mthTracer(
        /* pTrcAdminObj       */ m_pTrcAdminObj,
    ...
    if (m_pGraphObjConnectionLine == nullptr) {
        ...
        m_physValPolygon = m_pGraphObjConnectionLine->getPolygon(drawingSize.unit());
    }
    if (m_eDimensionUnit == EScaleDimensionUnit::Pixels) {
        if (drawingSize.dimensionUnit() == EScaleDimensionUnit::Metric) {
            // No simple unit conversion is possible here. The Y Scale Axis may
...
//------------------------------------------------------------------------------
QList<CModelGraphObjConnectionLineGeometry::SLabelSettings> CModelGraphObjConnectionLineGeometry::getLabelSettings(CGraphObj* i_pGraphObj) const
//------------------------------------------------------------------------------
{
...
//------------------------------------------------------------------------------
void CModelGraphObjConnectionLineGeometry::updateXYValueSizeHint()
//------------------------------------------------------------------------------
{
    CMethodTracer mthTracer(
        /* pTrcAdminObj       */ m_pTrcAdminObj,
...
*/
void CModelGraphObjConnectionLineGeometry::emit_contentChanged()
//------------------------------------------------------------------------------
{
    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObj,
...
//------------------------------------------------------------------------------
void CModelGraphObjConnectionLineGeometry::_beginResetModel()
//------------------------------------------------------------------------------
{
    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObj,
...
//------------------------------------------------------------------------------
void CModelGraphObjConnectionLineGeometry::_endResetModel()
//------------------------------------------------------------------------------
{
    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObj,
...
//------------------------------------------------------------------------------
void CModelGraphObjConnectionLineGeometry::_beginInsertRows(
...
//------------------------------------------------------------------------------
void CModelGraphObjConnectionLineGeometry::_endInsertRows()
//------------------------------------------------------------------------------
{
    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObj,
...
//------------------------------------------------------------------------------
void CModelGraphObjConnectionLineGeometry::_beginRemoveRows(
    const QModelIndex& i_modelIdxParent, int i_iRowFirst, int i_iRowLast)
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
...
//------------------------------------------------------------------------------
void CModelGraphObjConnectionLineGeometry::_endRemoveRows()
//------------------------------------------------------------------------------
{
    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObj,
...
//------------------------------------------------------------------------------
void CModelGraphObjConnectionLineGeometry::emit_dataChanged(
