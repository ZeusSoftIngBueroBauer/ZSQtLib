...
{
    m_pTrcAdminObj = CTrcServer::GetTraceAdminObj("ZS::Diagram", "CDiagScale", m_strObjName);

    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObj,
        /* iDatailLevel */ c_iTrcDetailLevel0,
        /* strMethod    */ "ctor",
        /* strAddInfo   */ "" );
...
//------------------------------------------------------------------------------
{
    if( m_scale != i_scale ) //lint !e1702
    {
        ...
        {
            strTrcMsg  = "Min=";
            strTrcMsg += QString::number(i_scale.m_fMin) + ", ";
            strTrcMsg += "Max=";
            strTrcMsg += QString::number(i_scale.m_fMax) + ", ";
            strTrcMsg += "Unit=";
            strTrcMsg += getSymbol(i_scale.m_pUnit);
        }
...
//------------------------------------------------------------------------------
{
    ...
    if( pUnit == nullptr )
    {
        pUnit = m_scale.m_pUnit;
    }
    if( !areOfSameUnitGroup(m_scale.m_pUnit,pUnit) )
    {
    ...
    if( pUnit != nullptr && m_scale.m_pUnit != nullptr && pUnit != m_scale.m_pUnit )
    {
        ...
        bConvertible = pUnit->isConvertible(m_scale.m_pUnit, 0.0);
    }
    if( bConvertible )
    {
        SScale scaleZoom = m_scale;
...
//------------------------------------------------------------------------------
{
    if( m_iScaleMinValPix != i_iScaleMinValPix || m_iScaleMaxValPix != i_iScaleMaxValPix )
    {
        ...
        {
            strTrcMsg  = "MinValPix=";
            strTrcMsg += QString::number(i_iScaleMinValPix) + ",";
            strTrcMsg += "MaxValPix=";
            strTrcMsg += QString::number(i_iScaleMaxValPix);
        }

        CMethodTracer mthTracer(
            /* pAdminObj    */ m_pTrcAdminObj,
            /* iDatailLevel */ c_iTrcDetailLevelLayout,
...
//------------------------------------------------------------------------------
{
    ...
    if( m_scale.m_pUnit != nullptr && i_pUnit != nullptr && i_pUnit != m_scale.m_pUnit )
    {
        fPixRes = m_scale.m_pUnit->convertValue( fPixRes, pUnit );
    }
    return fPixRes;
...
//------------------------------------------------------------------------------
{
    ...
    if( pUnit != nullptr && m_scale.m_pUnit != nullptr && pUnit != m_scale.m_pUnit )
    {
        fRes = m_scale.m_pUnit->convertValue( fRes, pUnit );
    }
    return fRes;
...
//------------------------------------------------------------------------------
{
    ...
    else
    {
        if( pUnit != nullptr && m_scale.m_pUnit != nullptr && pUnit != m_scale.m_pUnit )
        {
            fVal = pUnit->convertValue( fVal, m_scale.m_pUnit );
        }
...
//------------------------------------------------------------------------------
{
    ...
    if( i_iLayer >= 0 && i_iLayer < EDivLineLayerCount )
    {
        if( getScaleRangePix() > 1 && isScaleValid() && areDivLinesCalculated() )
        {
            ...
            if( pUnit != nullptr && m_scale.m_pUnit != nullptr && pUnit != m_scale.m_pUnit )
            {
                fDivLineVal = m_scale.m_pUnit->convertValue( fDivLineVal, pUnit );
            }
        }
    }
    return fDivLineVal;
...
//------------------------------------------------------------------------------
{
    ...
    if( getScaleRangePix() > 1 && isScaleValid() )
    {
        ...
        if( pUnit != nullptr && m_scale.m_pUnit != nullptr && pUnit != m_scale.m_pUnit )
        {
            fVal = pUnit->convertValue( i_fVal, m_scale.m_pUnit );
        }
...
//------------------------------------------------------------------------------
{
    ...
    if( isScaleValid() )
    {
        ...
        double fScaleRes = getScaleRes(i_fVal,pUnit);
...
//------------------------------------------------------------------------------
{
    ...
    if( pUnit != nullptr && m_scale.m_pUnit != nullptr && pUnit != m_scale.m_pUnit )
    {
        fVal = m_scale.m_pUnit->convertValue( fVal, pUnit );
    }
    return fVal;
...
//------------------------------------------------------------------------------
{
    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObj,
        /* iDatailLevel */ c_iTrcDetailLevelUpdate,
        /* strMethod    */ "update",
        /* strAddInfo   */ "" );
