    ...
    if (!i_xmlStreamReader.hasError()) {
        for (const SAnchorLayoutDscr& layoutDscr : arTextLabels) {
            if (!pGraphObj->isLabelAdded(layoutDscr.m_strKey)) {
                ...
                if (layoutDscr.m_selPt1.m_selPtType == ESelectionPointType::BoundingRectangle) {
                    pGraphObj->setLabelAnchorPoint(layoutDscr.m_strKey, layoutDscr.m_selPt1.m_selPtType, layoutDscr.m_selPt1.m_selPt);
                }
                else {
                    pGraphObj->setLabelAnchorPoint(layoutDscr.m_strKey, layoutDscr.m_selPt1.m_selPtType, layoutDscr.m_selPt1.m_idxPt);
                }
            }
