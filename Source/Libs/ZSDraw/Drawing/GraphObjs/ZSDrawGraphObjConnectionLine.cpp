...
//------------------------------------------------------------------------------
{
    ...
    if (m_editMode == EEditMode::CreatingByMouseEvents) {
        ...
        bool bIsPolylineHit = isPolylineHit(polygon(), ptEvLocalPos, m_pDrawingScene->getHitToleranceInPx(), &hitInfo);
        if (mthTracer.areMethodCallsActive(EMethodTraceDetailLevel::ArgsNormal) && mthTracer.isRuntimeInfoActive(ELogDetailLevel::Debug)) {
            mthTracer.trace("+- isPolylineHit(HitInfo {" + hitInfo.toString() + "}): " + bool2Str(bIsPolylineHit));
        }
        if ((polygon().size() > 2) && hitInfo.isPolygonShapePointHit()) {
            bool bFirstOrLastPolygonPointHit =
                (hitInfo.m_idxPolygonShapePoint == 0) || (hitInfo.m_idxPolygonShapePoint == (polygon().size()-1));
            if (bFirstOrLastPolygonPointHit) {
                m_pActionMenuContextDeletePoint->setEnabled(false);
            }
            else {
    ...
    if (pGraphicsItemThis != nullptr) {
        if (m_editMode == EEditMode::CreatingByMouseEvents) {
            ...
            if (pGraphObjSelPtHit != nullptr) {
                ...
                bool bIsPolylineHit = isPolylineHit(polygon(), i_pt, m_pDrawingScene->getHitToleranceInPx(), &hitInfo);
                if (mthTracer.areMethodCallsActive(EMethodTraceDetailLevel::ArgsNormal) && mthTracer.isRuntimeInfoActive(ELogDetailLevel::Debug)) {
                    mthTracer.trace("+- isPolylineHit(HitInfo {" + hitInfo.toString() + "}): " + bool2Str(bIsPolylineHit));
                }
                if (bIsPolylineHit) {
                    bool bFirstOrLastPolygonPointHit = false;
                    if ((polygon().size() > 2) && hitInfo.isPolygonShapePointHit()) {
                        bFirstOrLastPolygonPointHit =
                            (hitInfo.m_idxPolygonShapePoint == 0) || (hitInfo.m_idxPolygonShapePoint == (polygon().size()-1));
                    }
                    if (!bFirstOrLastPolygonPointHit) {
...
//------------------------------------------------------------------------------
{
    ...
    if (m_bPaintShapePath) {
        paintShapePath(i_pPainter);
    }
    else {
    ...
    if (m_bPaintBoundingRect) {
        paintBoundingRect(i_pPainter);
    }
    i_pPainter->restore();
}
...
//------------------------------------------------------------------------------
{
    ...
    if (m_pDrawingScene->getCurrentDrawingTool() == nullptr) {
        ...
        bool bIsPolylineHit = isPolylineHit(polygon(), i_pEv->pos(), m_pDrawingScene->getHitToleranceInPx(), &hitInfo);
        if (mthTracer.areMethodCallsActive(EMethodTraceDetailLevel::ArgsNormal) && mthTracer.isRuntimeInfoActive(ELogDetailLevel::Debug)) {
            mthTracer.trace("+- isPolylineHit(HitInfo {" + hitInfo.toString() + "}): " + bool2Str(bIsPolylineHit));
        }
        if (bIsPolylineHit) {
...
//------------------------------------------------------------------------------
{
    ...
    if (i_pEv->button() == Qt::LeftButton) {
        ...
        if (m_editMode == EEditMode::None) {
            if (bIsPolylineHit && i_pEv->modifiers() == Qt::NoModifier || i_pEv->modifiers() & Qt::ControlModifier) {
...
//------------------------------------------------------------------------------
{
    ...
    if (m_editMode == EEditMode::None) {
        ...
        // polygon point will be duplicated and appended. The duplicated polygon point
        // becomes the polygon point to be moved while creating the connection line.
        // Exception to this rule is the first mouse release right after creating the object.
        // The first mouse release event has to be ignored as already two polygon points at
        // the same position are existing.
        ...
        // When not clicking on a connection point and creating the line is not finished,
...
//------------------------------------------------------------------------------
{
    ...
    // Connection lines can't be moved. Only the connection points or the shape points may be moved.
...
//------------------------------------------------------------------------------
{
    ...
    if (m_idxsAdded.second > 0) {
        ...
        for (int idxPt = (iCountNew - 1); idxPt >= (m_idxsAdded.first + m_idxsAdded.second); --idxPt) {
            ...
            SAnchorLayoutDscr& labelDscrOld = m_hshLabelDscrs[strLabelNameOld];
            SAnchorLayoutDscr& labelDscrNew = m_hshLabelDscrs[strLabelNameNew];
        ...
        if (iCountPrev > 1) {
            for (const QString& strLabelName : m_hshLabelDscrs.keys()) {
                if (!isPolygonPointLabelName(strLabelName)) {
                    ...
                    SAnchorLayoutDscr& labelDscr = m_hshLabelDscrs[strLabelName];
        ...
        for (int idxPt = (iCountNew - 1); idxPt >= (m_idxsAdded.first + m_idxsAdded.second); --idxPt) {
            const QString strLabelNameOld = createPolygonPointLabelName(idxPt - m_idxsAdded.second);
            const QString strLabelNameNew = createPolygonPointLabelName(idxPt);
            SAnchorLayoutDscr& labelDscrOld = m_hshGeometryLabelDscrs[strLabelNameOld];
            SAnchorLayoutDscr& labelDscrNew = m_hshGeometryLabelDscrs[strLabelNameNew];
            labelDscrNew.m_bIsVisible = labelDscrOld.m_bIsVisible;
            labelDscrNew.m_bShowAnchorLine = labelDscrOld.m_bShowAnchorLine;
            labelDscrNew.m_polarCoorsToLinkedSelPt = labelDscrOld.m_polarCoorsToLinkedSelPt;
...
//------------------------------------------------------------------------------
{
    ...
    if (m_idxsRemoved.second > 0) {
        ...
        for (int idxPt = m_idxsRemoved.first; idxPt < iCountNew; ++idxPt) {
            ...
            SAnchorLayoutDscr labelDscr = m_hshLabelDscrs[strLabelNameOld];
        ...
        for (int idxPt = m_idxsRemoved.first; idxPt < iCountNew; ++idxPt) {
            const QString strLabelNameNew = createPolygonPointLabelName(idxPt);
            const QString strLabelNameOld = createPolygonPointLabelName(idxPt + m_idxsRemoved.second);
            SAnchorLayoutDscr labelDscr = m_hshGeometryLabelDscrs[strLabelNameOld];
            labelDscr.m_strKey = strLabelNameNew;
            labelDscr.m_strText = strLabelNameNew;
            labelDscr.m_selPt1.m_idxPt = idxPt;
