    ...
    switch( i_modelIdx.column() )
    {
        case EColumnNodeName:
        {
            ...
            else if( i_iRole == Qt::DecorationRole )
            {
                if( pNode->m_pUnit != nullptr )
                {
                    varData = CModelObjPool::GetIcon(EObjPoolEntryTypeObject);
                }
                else if( pNode->m_pUnitGrp != nullptr )
                {
                    varData = CModelObjPool::GetIcon(EObjPoolEntryTypeNameSpace);
                }
                else
                {
                    varData = CModelObjPool::GetIcon(EObjPoolEntryTypeRoot);
                }
            }
            break;
        }
