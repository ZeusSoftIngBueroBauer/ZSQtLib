...
//------------------------------------------------------------------------------
{
    ...
    m_bYAxisTopDown = physValSizeOther.m_bYAxisTopDown;
...
//------------------------------------------------------------------------------
/*! @brief Writes the point to coordinates into a string in the format "w, h"[ unit.symbol].
...
*/
QString CPhysValSize::toString(bool i_bAddUnit, const QString& i_strSeparator, int i_iPrecision) const
//------------------------------------------------------------------------------
{
    QString str;
    if (i_iPrecision < 0) {
        str = width().toString(EUnitFind::None, PhysValSubStr::Val)
            + i_strSeparator
            + height().toString(EUnitFind::None, PhysValSubStr::Val);
    }
    else {
        str = QString::number(Math::round2Nearest(width().getVal(), i_iPrecision), 'f', i_iPrecision)
            + i_strSeparator
            + QString::number(Math::round2Nearest(height().getVal(), i_iPrecision), 'f', i_iPrecision);
    }
    if (i_bAddUnit) {
        str += " " + m_unit.symbol();
    }
