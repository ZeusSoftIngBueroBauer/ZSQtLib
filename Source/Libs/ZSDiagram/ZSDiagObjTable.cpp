...
//------------------------------------------------------------------------------
void CDiagObjTable::setCellData( int i_idxRow, int i_idxClm, CUnit* i_pUnit )
//------------------------------------------------------------------------------
{
    if( i_idxRow < -1 || i_idxRow >= m_iRowCountMax
     || i_idxClm < -1 || i_idxClm >= m_iClmCountMax )
    ...
    for( idxRow = idxRowMin; idxRow <= idxRowMax; idxRow++ )
    {
        for( idxClm = idxClmMin; idxClm <= idxClmMax; idxClm++ )
        {
            ...
            if( m_ararpPhysValCellData[idxCell] == nullptr )
            {
                m_ararpPhysValCellData[idxCell] = new CPhysVal(i_pUnit);
            }
            else
            {
                m_ararpPhysValCellData[idxCell]->setUnit(i_pUnit);
            }
            m_ararvalidityCellData[idxCell] = EValueValidity::Valid;
        }
    }
...
//------------------------------------------------------------------------------
{
    ...
    if( m_ararpPhysValCellData[idxCell] != nullptr )
    {
        pUnit = m_ararpPhysValCellData[idxCell]->getUnit();
    }
    return pUnit;
        ...
        for( idxClm = 0; idxClm < m_iClmCount; idxClm++ )
        {
            ...
            switch( dataTypeCellDpl )
            {
                ...
                case EDataTypePhysUnit:
                {
                    ...
                    {
                        if( m_ararpPhysValCellData[idxCell]->getUnit() != nullptr )
                        {
                            m_ararstrCellData[idxCell] = m_ararpPhysValCellData[idxCell]->getUnit()->getSymbol();
                        }
                        else
                        {
