...
//------------------------------------------------------------------------------
    ...
    m_pUnitLabels(i_pDiagScale->getScale().m_pUnit),
    m_strPhysUnitLabels(),
...
//------------------------------------------------------------------------------
{
    ...
    pDiagObj->m_pUnit = m_pUnit;       // nullptr means "use best unit"
    pDiagObj->m_pUnitLabels = m_pUnitLabels; // Used to indicate the values (may differ from "m_pUnit")
...
//------------------------------------------------------------------------------
{
    ...
    if( m_pUnit != nullptr )
    {
        m_pUnitLabels = m_pUnit;
    }
    // If the "best" unit to indicate the values should be used ...
    else
    {
        CUnit* pUnitScaleMinValTmp;
        CUnit* pUnitScaleMaxValTmp;
        ...
        if( !(formatResult & EFormatResultError) ) //lint !e655
        {
            formatResult = PhysVal::formatValue(
                /* fValue                */ fScaleMaxVal,
                /* pUnitValue            */ m_pDiagScale->getScale().m_pUnit,
                /* iDigitsMantissaMax    */ 0,
                /* bAccuracyLimitsMant.  */ false,
                /* iDigitsAccuracy       */ 0,
                /* iDigitsExponent       */ 0,
            ...
            if( !(formatResult & EFormatResultError) ) //lint !e655
            {
                ...
                if( pUnitScaleMinValTmp == pUnitScaleMaxValTmp )
                {
                    m_pUnitLabels = pUnitScaleMinValTmp;
                }
                ...
                else
                {
                    ...
                    if( fabs(fScaleMinVal) == 0.0 )
                    {
                        ...
                        m_pUnitLabels = pUnitScaleMaxValTmp;
                    }
    ...
    if( m_pUnitLabels != nullptr )
    {
        m_strPhysUnitLabels = m_pUnitLabels->getSymbol();
    }
    else
    {
        m_strPhysUnitLabels = "";
    ...
    if( m_pUnitLabels != nullptr && m_pDiagScale->getScale().m_pUnit != nullptr )
    {
        fScaleMinVal = m_pDiagScale->getScale().m_pUnit->convertValue( m_pDiagScale->getScale().m_fMin, m_pUnitLabels );
        fScaleMaxVal = m_pDiagScale->getScale().m_pUnit->convertValue( m_pDiagScale->getScale().m_fMax, m_pUnitLabels );
    }
    for( iLayer = 0; iLayer < EDivLineLayerCount; iLayer++ )
    {
    ...
    if( iDivLinesCount >= 2 )
    {
        ...
        for( iLayer = 0; iLayer < EDivLineLayerCount; iLayer++ )
        {
            ...
            {
                fTmp = fabs(m_pDiagScale->getDivLineVal(iLayer,idxDivLine,m_pUnitLabels));
    ...
    if( iDivLinesCount < 2 && m_pDiagScale->isScaleValid() && fScaleMaxVal > fScaleMinVal )
    {
        ...
        fScaleRes = m_pDiagScale->getScaleRes(fDivLineValAbsMin,m_pUnitLabels);
        ...
        for( iLayer = 0; iLayer < EDivLineLayerCount; iLayer++ )
        {
            ...
            if( m_ariDivLinesCount[iLayer] == 1 )
            {
                ...
                fDivLineVal = m_pDiagScale->getDivLineVal(iLayer,0,m_pUnitLabels);
    ...
    else if( iDivLinesCount >= 2 && m_pDiagScale->isScaleValid() && fScaleMaxVal > fScaleMinVal )
    {
        ...
        while( iPrecisionMin != iPrecisionMinPrev )
        {
            ...
            for( fDivLineVal1 = fScaleMaxVal, iLayer = 0, iLayerTmp = 0; iLayerTmp < EDivLineLayerCount; iLayerTmp++ )
            {
                ...
                {
                    fDivLineVal = m_pDiagScale->getDivLineVal(iLayerTmp,0,m_pUnitLabels);
            ...
            for( idxDivLine = 1; idxDivLine < iDivLinesCount; idxDivLine++ )
            {
                ...
                for( fDivLineVal2 = fScaleMaxVal, iLayer = 0, iLayerTmp = 0; iLayerTmp < EDivLineLayerCount; iLayerTmp++ )
                {
                    ...
                    {
                        fDivLineVal = m_pDiagScale->getDivLineVal(iLayerTmp,aridxDivLine[iLayerTmp],m_pUnitLabels);
                ...
                if( fDivLineDist1 < fDivLineDist2 - m_pDiagScale->getScaleRes(m_pUnitLabels)
                 || fDivLineDist1 > fDivLineDist2 + m_pDiagScale->getScaleRes(m_pUnitLabels) )
                {
                    iPrecisionMin++;
                    break; // for( idxDivLine < uDivLineCount )
                }
        ...
        case ELayoutPosBottom:
        {
            ...
            for( iLayer = 0; iLayer < EDivLineLayerCount; iLayer++ )
            {
                if( m_ariDivLinesCount[iLayer] > 0 && m_arpRectDivLineLabels[iLayer] != nullptr && m_arpStrDivLineLabels[iLayer] != nullptr )
                {
                    ...
                    {
                        xDivLine = static_cast<int>(m_pDiagScale->getDivLinePix(iLayer,idxDivLine)+0.5);
                        strDivLineLabel = formatValue( m_pDiagScale->getDivLineVal(iLayer,idxDivLine,m_pUnitLabels) );
...
//------------------------------------------------------------------------------
{
    ...
    if( pUnit != nullptr && m_pUnitLabels != nullptr && pUnit != m_pUnitLabels )
    {
        fVal = pUnit->convertValue( fVal, m_pUnitLabels );
    }
    if( m_iDivLineLabelsExponentDigits <= 0 )
    {
