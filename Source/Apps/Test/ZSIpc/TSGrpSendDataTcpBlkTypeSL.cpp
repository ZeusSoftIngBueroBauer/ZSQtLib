...
//------------------------------------------------------------------------------
void CTest::FctDoTestStepGroupSendDataTcpBlkTypeSL( ZS::Test::CTest* i_pThis, ZS::Test::CTestStepGroup* i_pTestStepGrpParent )
//------------------------------------------------------------------------------
{
    dynamic_cast<CTest*>(i_pThis)->doTestStepGroupSendDataTcpBlkTypeSL(i_pTestStepGrpParent);
}
...
//------------------------------------------------------------------------------
void CTest::doTestStepGroupSendDataTcpBlkTypeSL( ZS::Test::CTestStepGroup* i_pTestStepGrpParent )
//------------------------------------------------------------------------------
{
    ZS::Test::CTestStepGroup* pTSGrp = getTestStepGroup( QString::number(m_iTestStepGroup) + ".SendData.Tcp.BlkType.SL", i_pTestStepGrpParent );
    ...
    else // if( pTSGrp->isEnabled() )
    {
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.SL.Server.Create", pTSGrp );
            //----------------------------------------------------------------------
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.SL.Server.Startup", pTSGrp );
            //----------------------------------------------------------------------
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.SL.Client.Create", pTSGrp );
            //----------------------------------------------------------------------
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.SL.Client.Connect", pTSGrp );
            //----------------------------------------------------------------------
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.SL.SetBlkType.Server", pTSGrp );
            //----------------------------------------------------------------------
            ...
            else // if( pTestStep->isEnabled() )
            {
                ...
                strDesiredValue += ":" + QString::number(m_serverHostSettings.m_uLocalPort);
                strDesiredValue += ":" + ZS::Ipc::socketType2Str(m_serverHostSettings.m_socketType);
                strDesiredValue += ":" + ZS::Ipc::blkType2Str(ZS::Ipc::EBlkTypeSL) + ")";
                strlstDesiredValues.append(strDesiredValue);
                strDesiredValue.clear();
                ...
                try
                {
                    ZS::Ipc::CBlkType* pBlkType = new ZS::Ipc::CBlkTypeSL(
                        /* byteArrBlkStart     */ "#SL98765432100123456789SL#",
                        /* blkLenDataType      */ ZS::Ipc::EBlkLenDataTypeStrBase10UINT32,
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.SL.SetBlkType.Client", pTSGrp );
            //----------------------------------------------------------------------
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.SL.SendData.CltSrv", pTSGrp );
            //----------------------------------------------------------------------
            ...
            else // if( pTestStep->isEnabled() )
            {
                ...
                try
                {
                    if( !QObject::connect(
                        ...
                        /* pObjReceiver */ this,
                        /* szSlot       */ SLOT(onTestStepGroupSendDataTcpBlkTypeSLServerReceivedData(QObject*,int,const QByteArray&)) ) )
                    {
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.SL.SendData.SrvClt", pTSGrp );
            //----------------------------------------------------------------------
            ...
            else // if( pTestStep->isEnabled() )
            {
                ...
                try
                {
                    if( !QObject::connect(
                        ...
                        /* pObjReceiver */ this,
                        /* szSlot       */ SLOT(onTestStepGroupSendDataTcpBlkTypeSLClientReceivedData(QObject*,const QByteArray&)) ) )
                    {
                        throw ZS::System::CException( __FILE__, __LINE__, EResultSignalSlotConnectionFailed );
                    }
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.SL.Server.Delete", pTSGrp );
            //----------------------------------------------------------------------
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.SL.Client.Disconnected", pTSGrp );
            //----------------------------------------------------------------------
            ...
            else // if( pTestStep->isEnabled() )
            {
                ...
                try
                {
                    if( !QObject::connect(
                        ...
                        /* pObjReceiver */ this,
                        /* szSlot       */ SLOT(onTestStepGroupSendDataTcpBlkTypeSLClientDisconnected(QObject*)) ) )
                    {
                        throw ZS::System::CException( __FILE__, __LINE__, EResultSignalSlotConnectionFailed );
                    }
                    if( m_pClient->getState() == ZS::Ipc::CClient::EStateUnconnected )
                    {
                        onTestStepGroupSendDataTcpBlkTypeSLClientDisconnected(m_pClient);
                    }
                }
                catch( CException& exc )
                {
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.SL.Client.Delete", pTSGrp );
            //----------------------------------------------------------------------
...
} // doTestStepGroupSendDataTcpBlkTypeSL
...
//------------------------------------------------------------------------------
void CTest::onTestStepGroupSendDataTcpBlkTypeSLServerReceivedData(
    QObject*          /*i_pServer*/,
    int               /*i_iSocketId*/,
...
//------------------------------------------------------------------------------
{
    ...
    ZS::Test::CTestStepGroup* pTSGrp = getTestStepGroup( QString::number(m_iTestStepGroup) + ".SendData.Tcp.BlkType.SL", getTestStepGroupRoot() );
    ZS::Test::CTestStep* pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(ETestStepSendDataCltSrv) + ".SendData.Tcp.BlkType.SL.SendData.CltSrv", pTSGrp );
...
} // onTestStepGroupSendDataTcpBlkTypeSLServerReceivedData

//------------------------------------------------------------------------------
void CTest::onTestStepGroupSendDataTcpBlkTypeSLClientReceivedData(
    QObject*          /*i_pClient*/,
    const QByteArray& i_byteArr )
//------------------------------------------------------------------------------
{
    ...
    ZS::Test::CTestStepGroup* pTSGrp = getTestStepGroup( QString::number(m_iTestStepGroup) + ".SendData.Tcp.BlkType.SL", getTestStepGroupRoot() );
    ZS::Test::CTestStep* pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(ETestStepSendDataSrvClt) + ".SendData.Tcp.BlkType.SL.SendData.SrvClt", pTSGrp );
...
} // onTestStepGroupSendDataTcpBlkTypeSLClientReceivedData

//------------------------------------------------------------------------------
void CTest::onTestStepGroupSendDataTcpBlkTypeSLClientDisconnected( QObject* /*i_pClient*/ )
//------------------------------------------------------------------------------
{
    if( !QObject::disconnect(
        /* pObjSender   */ m_pClient,
    ...
    ZS::Test::CTestStepGroup* pTSGrp = getTestStepGroup( QString::number(m_iTestStepGroup) + ".SendData.Tcp.BlkType.SL", getTestStepGroupRoot() );
    ZS::Test::CTestStep* pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(ETestStepClientDisconnected) + ".SendData.Tcp.BlkType.SL.Client.Disconnected", pTSGrp );
...
} // onTestStepGroupSendDataTcpBlkTypeSLClientDisconnected
