...
{
    m_pTrcAdminObj = CTrcServer::GetTraceAdminObj("ZS::Diagram", "CDiagTrace", m_strObjName);
    ...
    {
        m_arphysValRes[EScaleDirX].setUnit(i_pDiagScaleX->getScale().m_pUnit);
        m_arphysValArr[EScaleDirX].setUnit(i_pDiagScaleX->getScale().m_pUnit);
    }
    if( i_pDiagScaleY != nullptr )
    {
        m_arphysValRes[EScaleDirY].setUnit(i_pDiagScaleY->getScale().m_pUnit);
        m_arphysValArr[EScaleDirY].setUnit(i_pDiagScaleY->getScale().m_pUnit);
    }
...
//------------------------------------------------------------------------------
{
    ...
    if( i_pUnit != nullptr && pDiagScale->getScale().m_pUnit != nullptr && i_pUnit != pDiagScale->getScale().m_pUnit )
    {
        fRes = pDiagScale->getScale().m_pUnit->convertValue( fRes, i_pUnit );
    }
    return fRes;
...
//------------------------------------------------------------------------------
{
    ...
    if( i_physValArr.getUnit() != nullptr && pDiagScale->getScale().m_pUnit != nullptr && pDiagScale->getScale().m_pUnit != i_physValArr.getUnit() )
    {
        m_arphysValArr[i_scaleDir].convertValues(pDiagScale->getScale().m_pUnit);
    }
...
//------------------------------------------------------------------------------
{
    ...
    if( pUnitVal == nullptr )
    {
        pUnitVal = pDiagScale->getScale().m_pUnit;
    }

    if( !areOfSameUnitGroup(m_arphysValArr[i_scaleDir].getUnitGroup(),pUnitVal) )
    {
        m_arphysValArr->setUnit(pUnitVal);
    }

    if( i_pUnitRes == nullptr )
    {
        pUnitRes = pUnitVal;
    }
    ...
    if( fRes > 0.0 )
    {
        if( pUnitRes != nullptr && pDiagScale->getScale().m_pUnit != nullptr && pDiagScale->getScale().m_pUnit != pUnitRes )
        {
            fRes = pUnitRes->convertValue( fRes, pDiagScale->getScale().m_pUnit );
        }
        m_arphysValArr[i_scaleDir].setRes( CPhysValRes(fRes,pDiagScale->getScale().m_pUnit) );
    }

    m_arphysValArr[i_scaleDir].setValues( 0, i_arfValues, pUnitVal );

    if( pUnitVal != nullptr && pDiagScale->getScale().m_pUnit != nullptr && pDiagScale->getScale().m_pUnit != pUnitVal )
    {
        m_arphysValArr[i_scaleDir].convertValues(pDiagScale->getScale().m_pUnit);
    }
...
//------------------------------------------------------------------------------
{
    ...
    if( i_pUnitSrc != nullptr && pDiagScaleSrc->getScale().m_pUnit != nullptr && i_pUnitSrc != pDiagScaleSrc->getScale().m_pUnit )
    {
        fValSrc = i_pUnitSrc->convertValue( fValSrc, pDiagScaleSrc->getScale().m_pUnit );
    }
    ...
    if( o_pfValDst != nullptr )
    {
        ...
        if( i_pUnitDst != nullptr && pDiagScaleDst->getScale().m_pUnit != nullptr && i_pUnitDst != pDiagScaleDst->getScale().m_pUnit )
        {
            fValDst = pDiagScaleDst->getScale().m_pUnit->convertValue( fValDst, i_pUnitDst );
        }
        *o_pfValDst = fValDst;
    }
