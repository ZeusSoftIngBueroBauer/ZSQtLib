    ...
    m_pMainWindow(nullptr)
{
    ...
    QString strErrLogFileBaseName = strAppNameNormalized + "-ErrLog2"; // not the default name
    ...
    CErrLog::CreateInstance(); // Default path and default name "ZSErrLog"
...
//------------------------------------------------------------------------------
{
    if( m_pSettingsFile != nullptr )
    {
        ...
        if( m_pTest != nullptr )
        {
            ...
            m_pSettingsFile->setValue( strSettingsKey+"/AdminObjFileName", m_pTest->getAdminObjIdxTree()->getFileName() );
