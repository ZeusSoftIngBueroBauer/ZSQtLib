...
CObjPool*     CObjFactory::s_pObjPoolAllFactories = nullptr;
CObjPool*     CObjFactory::s_pObjPoolToolBoxFactories = nullptr;
...
//------------------------------------------------------------------------------
{
    ...
    if( s_pObjPoolAllFactories != nullptr && i_idx >= 0 && i_idx < static_cast<int>(s_pObjPoolAllFactories->getObjCount()) )
    {
        CObjPoolListEntry* pListEntry = s_pObjPoolAllFactories->getListEntry(i_idx);

        if( pListEntry != nullptr )
        {
            pObjFactory = reinterpret_cast<CObjFactory*>(pListEntry->getObj());
        }
    }
    return pObjFactory;
...
//------------------------------------------------------------------------------
{
    return reinterpret_cast<CObjFactory*>(s_pObjPoolAllFactories->getObj(i_strGraphObjPath));
}
...
//------------------------------------------------------------------------------
{
    ...
    if( s_pObjPoolAllFactories != nullptr )
    {
        for( idx = 0; idx < static_cast<int>(s_pObjPoolAllFactories->getObjCount()); idx++ )
        {
            CObjPoolListEntry* pListEntry = s_pObjPoolAllFactories->getListEntry(idx);

            if( pListEntry != nullptr )
            {
                pObjFactoryTmp = reinterpret_cast<CObjFactory*>(pListEntry->getObj());
...
//------------------------------------------------------------------------------
void CObjFactory::RegisterObjFactory( CObjFactory& i_objFactory, bool i_bAddToToolBoxObjPool )
//------------------------------------------------------------------------------
{
    ...
    int iId = s_pObjPoolAllFactories->addObjNode(
        /* strNameSpace */ strNameSpace,
        /* strObjName   */ &strName,
        /* pvObj        */ &i_objFactory );
    i_objFactory.setObjPoolIdAllFactories(iId);

    if( i_bAddToToolBoxObjPool )
    {
        iId = s_pObjPoolToolBoxFactories->addObjNode(
            /* strNameSpace */ strNameSpace,
            /* strObjName   */ &strName,
            /* pvObj        */ &i_objFactory );
        i_objFactory.setObjPoolIdToolBoxFactories(iId);
    }
...
//------------------------------------------------------------------------------
{
    ...
    s_pObjPoolAllFactories->invalidateObjNode( i_objFactory.getObjPoolIdAllFactories() );

    if( i_objFactory.getObjPoolIdToolBoxFactories() >= 0 )
    {
        s_pObjPoolToolBoxFactories->invalidateObjNode( i_objFactory.getObjPoolIdToolBoxFactories() );
    }
...
//------------------------------------------------------------------------------
    ...
    m_iObjPoolIdAllFactories(-1),
    m_iObjPoolIdToolBoxFactories(-1),
    m_strFileName(),
    m_pTrcAdminObj(nullptr)
{
    ...
    if( s_pObjPoolAllFactories == nullptr )
    {
        s_pObjPoolAllFactories = new CObjPool(
            /* strModelName           */ "All Object Factories",
            /* strNodeSeparator       */ "::",
            /* bSortTreeEntriesByName */ false );
    }

    if( s_pObjPoolToolBoxFactories == nullptr )
    {
        s_pObjPoolToolBoxFactories = new CObjPool(
            /* strModelName           */ "Tool Box",
            /* strNodeSeparator       */ "::",
            /* bSortTreeEntriesByName */ false );
    }
    ...
    RegisterObjFactory(*this,i_bAddToToolBoxObjPool);
...
//------------------------------------------------------------------------------
{
    ...
    if( s_iInstCount == 0 )
    {
        try
        {
            delete s_pObjPoolToolBoxFactories;
        }
        catch(...)
        {
        }
        s_pObjPoolToolBoxFactories = nullptr;

        try
        {
            delete s_pObjPoolAllFactories;
        }
        catch(...)
        {
        }
        s_pObjPoolAllFactories = nullptr;
    }
