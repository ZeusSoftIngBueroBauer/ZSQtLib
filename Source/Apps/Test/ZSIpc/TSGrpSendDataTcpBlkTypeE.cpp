...
//------------------------------------------------------------------------------
void CTest::FctDoTestStepGroupSendDataTcpBlkTypeE( ZS::Test::CTest* i_pThis, ZS::Test::CTestStepGroup* i_pTestStepGrpParent )
//------------------------------------------------------------------------------
{
    dynamic_cast<CTest*>(i_pThis)->doTestStepGroupSendDataTcpBlkTypeE(i_pTestStepGrpParent);
}
...
//------------------------------------------------------------------------------
void CTest::doTestStepGroupSendDataTcpBlkTypeE( ZS::Test::CTestStepGroup* i_pTestStepGrpParent )
//------------------------------------------------------------------------------
{
    ZS::Test::CTestStepGroup* pTSGrp = getTestStepGroup( QString::number(m_iTestStepGroup) + ".SendData.Tcp.BlkType.E", i_pTestStepGrpParent );
    ...
    else // if( pTSGrp->isEnabled() )
    {
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.E.Server.Create", pTSGrp );
            //----------------------------------------------------------------------
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.E.Server.Startup", pTSGrp );
            //----------------------------------------------------------------------
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.E.Client.Create", pTSGrp );
            //----------------------------------------------------------------------
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.E.Client.Connect", pTSGrp );
            //----------------------------------------------------------------------
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.E.SetBlkType.Server", pTSGrp );
            //----------------------------------------------------------------------
            ...
            else // if( pTestStep->isEnabled() )
            {
                ...
                strDesiredValue += ":" + QString::number(m_serverHostSettings.m_uLocalPort);
                strDesiredValue += ":" + ZS::Ipc::socketType2Str(m_serverHostSettings.m_socketType);
                strDesiredValue += ":" + ZS::Ipc::blkType2Str(ZS::Ipc::EBlkTypeE) + ")";
                strlstDesiredValues.append(strDesiredValue);
                strDesiredValue.clear();
                ...
                try
                {
                    ZS::Ipc::CBlkType* pBlkType = new ZS::Ipc::CBlkTypeE(
                        /* byteArrBlkEnd */ "#E98765432100123456789E#",
                        /* bInsertBlkEnd */ true,
                        /* bRemoveBlkEnd */ true );
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.E.SetBlkType.Client", pTSGrp );
            //----------------------------------------------------------------------
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.E.SendData.CltSrv", pTSGrp );
            //----------------------------------------------------------------------
            ...
            else // if( pTestStep->isEnabled() )
            {
                ...
                try
                {
                    if( !QObject::connect(
                        ...
                        /* pObjReceiver */ this,
                        /* szSlot       */ SLOT(onTestStepGroupSendDataTcpBlkTypeEServerReceivedData(QObject*,int,const QByteArray&)) ) )
                    {
            ...
            // Don't continue with next test step of this group but wait until the
            // server has received the data block emitting the "receivedData" signal.
            // m_iTestStep++;
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.E.SendData.SrvClt", pTSGrp );
            //----------------------------------------------------------------------
            ...
            else // if( pTestStep->isEnabled() )
            {
                ...
                try
                {
                    if( !QObject::connect(
                        ...
                        /* pObjReceiver */ this,
                        /* szSlot       */ SLOT(onTestStepGroupSendDataTcpBlkTypeEClientReceivedData(QObject*,const QByteArray&)) ) )
                    {
                        throw ZS::System::CException( __FILE__, __LINE__, EResultSignalSlotConnectionFailed );
                    }
            ...
            // Don't continue with next test step of this group but wait until the
            // client has received the data block emitting the "receivedData" signal.
            // m_iTestStep++;
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.E.Server.Delete", pTSGrp );
            //----------------------------------------------------------------------
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.E.Client.Disconnected", pTSGrp );
            //----------------------------------------------------------------------
            ...
            else // if( pTestStep->isEnabled() )
            {
                ...
                try
                {
                    if( !QObject::connect(
                        ...
                        /* pObjReceiver */ this,
                        /* szSlot       */ SLOT(onTestStepGroupSendDataTcpBlkTypeEClientDisconnected(QObject*)) ) )
                    {
                        throw ZS::System::CException( __FILE__, __LINE__, EResultSignalSlotConnectionFailed );
                    }
                    if( m_pClient->getState() == ZS::Ipc::CClient::EStateUnconnected )
                    {
                        onTestStepGroupSendDataTcpBlkTypeEClientDisconnected(m_pClient);
                    }
                }
                catch( CException& exc )
                {
        ...
        {
            //----------------------------------------------------------------------
            pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(m_iTestStep) + ".SendData.Tcp.BlkType.E.Client.Delete", pTSGrp );
            //----------------------------------------------------------------------
...
} // doTestStepGroupSendDataTcpBlkTypeE
...
//------------------------------------------------------------------------------
void CTest::onTestStepGroupSendDataTcpBlkTypeEServerReceivedData(
    QObject*          /*i_pServer*/,
    int               /*i_iSocketId*/,
...
//------------------------------------------------------------------------------
{
    ...
    ZS::Test::CTestStepGroup* pTSGrp = getTestStepGroup( QString::number(m_iTestStepGroup) + ".SendData.Tcp.BlkType.E", getTestStepGroupRoot() );
    ZS::Test::CTestStep* pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(ETestStepSendDataCltSrv) + ".SendData.Tcp.BlkType.E.SendData.CltSrv", pTSGrp );
...
} // onTestStepGroupSendDataTcpBlkTypeEServerReceivedData

//------------------------------------------------------------------------------
void CTest::onTestStepGroupSendDataTcpBlkTypeEClientReceivedData(
    QObject*          /*i_pClient*/,
    const QByteArray& i_byteArr )
//------------------------------------------------------------------------------
{
    ...
    ZS::Test::CTestStepGroup* pTSGrp = getTestStepGroup( QString::number(m_iTestStepGroup) + ".SendData.Tcp.BlkType.E", getTestStepGroupRoot() );
    ZS::Test::CTestStep* pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(ETestStepSendDataSrvClt) + ".SendData.Tcp.BlkType.E.SendData.SrvClt", pTSGrp );
...
} // onTestStepGroupSendDataTcpBlkTypeEClientReceivedData

//------------------------------------------------------------------------------
void CTest::onTestStepGroupSendDataTcpBlkTypeEClientDisconnected( QObject* /*i_pClient*/ )
//------------------------------------------------------------------------------
{
    if( !QObject::disconnect(
        /* pObjSender   */ m_pClient,
    ...
    ZS::Test::CTestStepGroup* pTSGrp = getTestStepGroup( QString::number(m_iTestStepGroup) + ".SendData.Tcp.BlkType.E", getTestStepGroupRoot() );
    ZS::Test::CTestStep* pTestStep = getTestStep( QString::number(m_iTestStepGroup) + "." + QString::number(ETestStepClientDisconnected) + ".SendData.Tcp.BlkType.E.Client.Disconnected", pTSGrp );
...
} // onTestStepGroupSendDataTcpBlkTypeEClientDisconnected
