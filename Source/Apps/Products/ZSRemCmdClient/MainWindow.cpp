...
//------------------------------------------------------------------------------
    ...
    m_pActSettingsRemCmdClient(nullptr),
    m_pMnuDebug(nullptr),
    ...
    m_pWdgtRemCmdClientCnctStatus(nullptr),
    m_pLblRemCmdClientCnctStatusIcon(nullptr),
    m_pWdgtCentral(nullptr),
    m_pDlgRemCmdClient(nullptr),
    m_pTrcAdminObj(nullptr)
{
    ...
    m_pTrcAdminObj = CTrcServer::GetTraceAdminObj("ZS::Apps::Products::RemCmdClient", "CMainWindow", objectName());
    ...
    if( !QObject::connect(
        ...
        /* pObjReceiver */ this,
        /* szSlot       */ SLOT(onRemCmdClientDestroyed(QObject*)) ) )
    {
    ...
    if( !QObject::connect(
        ...
        /* pObjReceiver */ this,
        /* szSlot       */ SLOT(onRemCmdClientStateChanged(QObject*,int)) ) )
    {
        throw ZS::System::CException( __FILE__, __LINE__, EResultSignalSlotConnectionFailed );
    }
    ...
    m_pActSettingsRemCmdClient = new QAction("&Remote Command Client",this);
    m_pMnuSettings->addAction(m_pActSettingsRemCmdClient);

    if( !connect(
        /* pObjSender   */ m_pActSettingsRemCmdClient,
        /* szSignal     */ SIGNAL(triggered()),
        /* pObjReceiver */ this,
        /* szSlot       */ SLOT(onActSettingsRemCmdClientTriggered()) ) )
    {
        throw ZS::System::CException(__FILE__,__LINE__,EResultSignalSlotConnectionFailed);
    }
    ...
    m_pWdgtRemCmdClientCnctStatus = new QWidget(this);

    new QHBoxLayout(m_pWdgtRemCmdClientCnctStatus);

    m_pWdgtRemCmdClientCnctStatus->setContentsMargins(0,0,0,0);
    m_pWdgtRemCmdClientCnctStatus->layout()->setContentsMargins(1,1,1,1);
    m_pWdgtRemCmdClientCnctStatus->installEventFilter(this);

    m_pLblRemCmdClientCnctStatusIcon = new QLabel("");
    m_pLblRemCmdClientCnctStatusIcon->setScaledContents(true);
    m_pLblRemCmdClientCnctStatusIcon->setMaximumSize( QSize(16,16) );
    m_pWdgtRemCmdClientCnctStatus->layout()->addWidget(m_pLblRemCmdClientCnctStatusIcon);

    statusBar()->addPermanentWidget(m_pWdgtRemCmdClientCnctStatus);

    onRemCmdClientStateChanged( m_pRemCmdClient, m_pRemCmdClient->getState() );

    // Central Widget
    //===============

    m_pWdgtCentral = new CWidgetCentral(i_pRemCmdClient);

    setCentralWidget(m_pWdgtCentral);

    if( !connect(
        /* pObjSender   */ m_pWdgtCentral,
        /* szSignal     */ SIGNAL(editRemoteSettingsTriggered()),
        /* pObjReceiver */ this,
        /* szSlot       */ SLOT(onActSettingsRemCmdClientTriggered()) ) )
    {
        throw ZS::System::CException(__FILE__,__LINE__,EResultSignalSlotConnectionFailed);
...
//------------------------------------------------------------------------------
{
    ...
    else if( i_pObjWatched == m_pWdgtRemCmdClientCnctStatus )
    {
        if( i_pEv->type() == QEvent::MouseButtonDblClick )
        {
            onActSettingsRemCmdClientTriggered();
            bHandled = true;
        }
    }
    else
...
//------------------------------------------------------------------------------
void CMainWindow::onActSettingsRemCmdClientTriggered()
//------------------------------------------------------------------------------
{
    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObj,
        /* iDetailLevel */ ETraceDetailLevelMethodCalls,
        /* strMethod    */ "onActSettingsRemCmdClientTriggered",
        /* strAddInfo   */ "" );

    QString strDlgTitle = windowTitle() + ": Remote Command Client";

    CDlgIpcClient* pDlg = CDlgIpcClient::GetInstance(strDlgTitle);

    if( m_pDlgRemCmdClient == nullptr )
    {
        m_pDlgRemCmdClient = CDlgIpcClient::CreateInstance(m_pRemCmdClient->objectName(), strDlgTitle);
        m_pDlgRemCmdClient->setClient(m_pRemCmdClient);
        m_pDlgRemCmdClient->adjustSize();
    }

    if( m_pDlgRemCmdClient->isHidden() )
    {
        m_pDlgRemCmdClient->show();
    }

    m_pDlgRemCmdClient->raise();
    m_pDlgRemCmdClient->activateWindow();
...
//------------------------------------------------------------------------------
void CMainWindow::onRemCmdClientStateChanged( QObject* i_pRemCmdClient, int i_iState )
//------------------------------------------------------------------------------
{
    ...
    if( m_pTrcAdminObj != nullptr && m_pTrcAdminObj->isActive(ETraceDetailLevelMethodArgs) )
    {
        ...
        {
            strAddTrcInfo = "RemCmdClient: nullptr";
        }
        else
        {
            strAddTrcInfo = "RemCmdClient: " + i_pRemCmdClient->objectName();
        }
        strAddTrcInfo += ", State: " + CRemCmdClient::State2Str(i_iState);
    }

    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObj,
        /* iDetailLevel */ ETraceDetailLevelMethodCalls,
        /* strMethod    */ "onRemCmdClientStateChanged",
    ...
    Ipc::SSocketDscr socketDscr = m_pRemCmdClient->getSocketDscr();
    ...
    else if( m_pRemCmdClient->requestInProgress() == CRemCmdClient::ERequestConnect )
    {
    ...
    if( m_pLblRemCmdClientCnctStatusIcon != nullptr )
    {
        m_pLblRemCmdClientCnctStatusIcon->setPixmap(pxmStatus);
        m_pLblRemCmdClientCnctStatusIcon->setToolTip(strToolTip);
    }

} // onRemCmdClientStateChanged

//------------------------------------------------------------------------------
void CMainWindow::onRemCmdClientDestroyed( QObject* i_pRemCmdClient )
//------------------------------------------------------------------------------
{
    QString strAddTrcInfo;
    ...
    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObj,
        /* iDetailLevel */ ETraceDetailLevelMethodCalls,
        /* strMethod    */ "onRemCmdClientDestroyed",
        /* strAddInfo   */ strAddTrcInfo );

    if( i_pRemCmdClient == m_pRemCmdClient )
    {
        m_pRemCmdClient = nullptr;
    }

} // onRemCmdClientDestroyed
