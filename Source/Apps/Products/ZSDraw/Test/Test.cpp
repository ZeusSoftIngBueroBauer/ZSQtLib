...
//------------------------------------------------------------------------------
{
    ...
    m_pPhysValPolygonConnectionLineRect1RightCenterRect4LeftCenter = nullptr;
    delete m_pPhysValPolygonConnectionLineRect3BottomCenterRect4TopCenter;
    ...
    delete m_pPhysValPolygonConnectionLineStar2RightCenterStar4LeftCenter;
    m_pPhysValPolygonConnectionLineStar2RightCenterStar4LeftCenter = nullptr;
    delete m_pPhysValPolygonConnectionLineStar4TopCenterStar3BottomCenter;
    m_pPhysValPolygonConnectionLineStar4TopCenterStar3BottomCenter = nullptr;
    delete m_pPhysValPolygonConnectionLineStar3LeftCenterStar1RightCenter;
    m_pPhysValPolygonConnectionLineStar3LeftCenterStar1RightCenter = nullptr;
    delete m_pPhysValPolygonConnectionLineStar1LineCenterPt1Pt2Star3LineCenterPt8Pt0;
    m_pPhysValPolygonConnectionLineStar1LineCenterPt1Pt2Star3LineCenterPt8Pt0 = nullptr;
...
//------------------------------------------------------------------------------
{
    ...
    m_pPhysValPolygonConnectionLineStar2RightCenterStar4LeftCenter = new CPhysValPolygon(*m_pDrawingScene);
    m_pPhysValPolygonConnectionLineStar4TopCenterStar3BottomCenter = new CPhysValPolygon(*m_pDrawingScene);
...
//------------------------------------------------------------------------------
{
    ...
    QPolygonF polygonLineStartArrowHead({
        QPointF(109.0, 103.0), QPointF(100.0, 100.0), QPointF(109.0, 97.0)
    });
    QPolygonF polygonLineEndArrowHead({
        QPointF(191.0, 97.0), QPointF(200.0, 100.0), QPointF(191.0, 103.0)
    });
    ...
    polygonLineStartArrowHead = QPolygonF({
        QPointF(97.0, 109.0), QPointF(100.0, 100.0), QPointF(103.0, 109.0)
    });
    polygonLineEndArrowHead = QPolygonF({
        QPointF(103.0, 191.0), QPointF(100.0, 200.0), QPointF(97.0, 191.0)
    });
    strlstExpectedValues.append(qPolygon2Str(polygonLineStartArrowHead, ", ", 'f', iPrecision));
    strlstExpectedValues.append(qPolygon2Str(polygonLineEndArrowHead, ", ", 'f', iPrecision));
    ...
    pTestStep->addDataRow({
        {"Line", QLineF(QPointF(100.0, 100.0), QPointF(200.0, 100.0))},
        {"ArrowHeadBaseLineType", CEnumArrowHeadBaseLineType(EArrowHeadBaseLineType::Indented).toString()},
        {"ArrowHeadWidth", CEnumArrowHeadWidth(EArrowHeadWidth::Thin).toString()},
        {"ArrowHeadLength", CEnumArrowHeadLength(EArrowHeadLength::Medium).toString()},
        {"ResultValuesPrecision", iPrecision}
    });
    polygonLineStartArrowHead = QPolygonF({
        QPointF(109.0, 103.0), QPointF(100.0, 100.0), QPointF(109.0, 97.0), QPointF(105.0, 100.0)
    });
    polygonLineEndArrowHead = QPolygonF({
        QPointF(191.0, 97.0), QPointF(200.0, 100.0), QPointF(191.0, 103.0), QPointF(195.0, 100.0)
    });
    strlstExpectedValues.append(qPolygon2Str(polygonLineStartArrowHead, ", ", 'f', iPrecision));
    strlstExpectedValues.append(qPolygon2Str(polygonLineEndArrowHead, ", ", 'f', iPrecision));
    // Vertical line, Indented
    pTestStep->addDataRow({
        {"Line", QLineF(QPointF(100.0, 100.0), QPointF(100.0, 200.0))},
        {"ArrowHeadBaseLineType", CEnumArrowHeadBaseLineType(EArrowHeadBaseLineType::Indented).toString()},
        {"ArrowHeadWidth", CEnumArrowHeadWidth(EArrowHeadWidth::Thin).toString()},
    ...
    polygonLineStartArrowHead = QPolygonF({
        QPointF(97.0, 109.0), QPointF(100.0, 100.0), QPointF(103.0, 109.0), QPointF(100.0, 105.0)
    });
    polygonLineEndArrowHead = QPolygonF({
        QPointF(103.0, 191.0), QPointF(100.0, 200.0), QPointF(97.0, 191.0), QPointF(100.0, 195.0)
    });
    strlstExpectedValues.append(qPolygon2Str(polygonLineStartArrowHead, ", ", 'f', iPrecision));
    strlstExpectedValues.append(qPolygon2Str(polygonLineEndArrowHead, ", ", 'f', iPrecision));
...
//------------------------------------------------------------------------------
{
    ...
    drawSettingsArrowHeads.setLineEndStyle(ELinePoint::End, ELineEndStyle::ArrowHead);
    int iPrecision = -1;
...
//------------------------------------------------------------------------------
{
    ...
    if (pGraphObjParent == nullptr) {
        ...
        if (pObjFactory != nullptr) {
            ...
            strGraphObjName = i_pTestStep->getConfigValue("GraphObjName").toString();
            pGraphObj->rename(strGraphObjName);
...
//------------------------------------------------------------------------------
void CTest::doTestStepDrawGraphObjConnectionLine(ZS::Test::CTestStep* i_pTestStep)
//------------------------------------------------------------------------------
{
    QString strMthInArgs;
    if (areMethodCallsActive(m_pTrcAdminObjDrawTestSteps, EMethodTraceDetailLevel::ArgsNormal)) {
    ...
    CMethodTracer mthTracer(
        /* pAdminObj    */ m_pTrcAdminObjDrawTestSteps,
        /* iDetailLevel */ EMethodTraceDetailLevel::EnterLeave,
        /* strMethod    */ "doTestStepDrawGraphObjConnectionLine",
        /* strAddInfo   */ strMthInArgs );
    ...
    if (strMethod == "setCurrentDrawingTool") {
        m_pDrawingScene->setCurrentDrawingTool(strFactoryGroupName, strGraphObjType);
        i_pTestStep->setConfigValue("Method", "mouseMoveEvent");
        // We need at least 3 points. The start position (from which we move to the first connection point)
        ...
        if (points.size() > 2) {
            i_pTestStep->setConfigValue("Points.Idx", 1);
            addMouseMoveEventDataRows(i_pTestStep, points[0], points[1]);
        }
        else {
            i_pTestStep->setConfigValue("Method", "setResultValues");
        }
        ...
        int idxPt = i_pTestStep->getConfigValue("Points.Idx").toInt();
        QPoint ptMousePos = points.at(idxPt);
        Qt::MouseButton mouseBtn = static_cast<Qt::MouseButton>(i_pTestStep->getConfigValue("MouseButtons").toInt());
        Qt::KeyboardModifiers keyboardModifiers = static_cast<Qt::KeyboardModifiers>(i_pTestStep->getConfigValue("KeyboardModifiers").toInt());
        QPoint ptDrawingViewMousePos = m_pDrawingView->mapFromScene(ptMousePos);
        QPoint ptMousePosGlobal = m_pDrawingView->mapToGlobal(ptDrawingViewMousePos);
        ptMousePosGlobal.setX(ptMousePosGlobal.x()+1); // Maybe graphics view or graphics scene bug on calculating the screen position.
        ptMousePosGlobal.setY(ptMousePosGlobal.y()+1); // Without adding 1 pixel the newly created object will not be selected by the scene.
        ...
        delete pMouseEv;
        pMouseEv = nullptr;
        if (idxPt > 0 && idxPt < (points.size()-1)) {
            i_pTestStep->setConfigValue("Method", "mouseMoveEvent");
            addMouseMoveEventDataRows(i_pTestStep, points[idxPt], points[idxPt+1]);
            i_pTestStep->setConfigValue("Points.Idx", idxPt + 1);
        }
        ...
        if (iMouseMovesRemaining > 0) {
            QHash<QString, QVariant> dataRow = i_pTestStep->takeDataRow(0);
            QPoint ptMouseMovePos = dataRow["MousePos"].toPoint();
            Qt::MouseButton mouseBtn = static_cast<Qt::MouseButton>(dataRow["MouseButtons"].toInt());
            Qt::KeyboardModifiers modifiers = static_cast<Qt::KeyboardModifiers>(dataRow["KeyboardModifiers"].toInt());
            QPoint ptDrawingViewMousePos = m_pDrawingView->mapFromScene(ptMouseMovePos);
            QPoint ptMousePosGlobal = m_pDrawingView->mapToGlobal(ptDrawingViewMousePos);
            QMouseEvent* pMouseEv = new QMouseEvent(
                /* type      */ QEvent::MouseMove,
...
//------------------------------------------------------------------------------
{
    ...
    *m_pPhysValPolygonConnectionLineStar2RightCenterStar4LeftCenter = CPhysValPolygon(*m_pDrawingScene);
    m_polygonConnectionLineStar4TopCenterStar3BottomCenter = QPolygonF();
    *m_pPhysValPolygonConnectionLineStar4TopCenterStar3BottomCenter = CPhysValPolygon(*m_pDrawingScene);
    m_polygonConnectionLineStar3LeftCenterStar1RightCenter = QPolygonF();
    *m_pPhysValPolygonConnectionLineStar3LeftCenterStar1RightCenter = CPhysValPolygon(*m_pDrawingScene);
