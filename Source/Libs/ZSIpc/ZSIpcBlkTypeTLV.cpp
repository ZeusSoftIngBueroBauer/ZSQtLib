    ...
    while( !bBlockReceived && pMsgErr == nullptr && i_pSocketWrapper->bytesAvailable() )
    {
        ...
        while( i_pSocketWrapper->bytesAvailable() < uTagSize )
        {
            ...
            if( !bReadyRead || (fCurrTime_ms - fStartTime_ms) > static_cast<double>(iReadTimeout_ms) )
            {
                ...
                POST_OR_DELETE_MESSAGE(pMsgErr, i_pMthTracer, ETraceDetailLevelRuntimeInfo::DebugNormal);
                break;
            }
        }
        ...
        if( pMsgErr == nullptr )
        {
            ...
            if( bIsWatchDogBlock )
            {
                if( i_pMthTracer != nullptr && i_pMthTracer->isRuntimeInfoActive(ETraceDetailLevelRuntimeInfo::DebugNormal) )
                {
                ...
                if( i_srvCltType == ESrvCltTypeServer )
                {
                    CMsgReqSendData* pMsgReq = new CMsgReqSendData(
                        ...
                        /* iReqId           */ -1 );
                    POST_OR_DELETE_MESSAGE(pMsgReq, i_pMthTracer, ETraceDetailLevelRuntimeInfo::DebugNormal);
                    pMsgReq = nullptr;
                }
