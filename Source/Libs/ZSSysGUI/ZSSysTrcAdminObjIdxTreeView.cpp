...
//------------------------------------------------------------------------------
{
    ...
    if (areMethodCallsActive(m_pTrcAdminObj, EMethodTraceDetailLevel::ArgsNormal)) {
        strMthInArgs = "RectVisual: " + qRect2Str(i_rectVisual) +
            ", Pos: " + qPoint2Str(i_ptPos) +
            ", ModelIdx {" + CModelIdxTreeTrcAdminObjs::modelIdx2Str(i_modelIdx) + "}";
    }
    CMethodTracer mthTracer(
        /* pTrcAdminObj       */ m_pTrcAdminObj,
        /* eFilterDetailLevel */ EMethodTraceDetailLevel::EnterLeave,
    ...
    if (i_rectVisual.width() <= m_rectChkBoxTraceEnabled.width() && i_rectVisual.height() <= m_rectChkBoxTraceEnabled.height()) {
        bIsHit = true;
    }
...
//------------------------------------------------------------------------------
{
    ...
    if (i_modelIdx.isValid()) {
        ...
        if (i_modelIdx.column() == CModelIdxTreeTrcAdminObjs::EColumnEnabled) {
...
//------------------------------------------------------------------------------
{
    ...
    if (i_modelIdx.isValid()) {
        ...
        switch (i_modelIdx.column()) {
            ...
            case CModelIdxTreeTrcAdminObjs::EColumnObjNameFilter: {
                if (pCModelTreeEntry->isLeave()) {
                    QLineEdit* pEdt = new QLineEdit(i_pWdgtParent);
                    pEdt->setObjectName(pCModelTreeEntry->keyInTree() + ".ObjectNameFilter");
                    pWdgtEditor = pEdt;
                    QObject::connect(
                        pEdt, &QLineEdit::editingFinished,
                        this, &CDelegateIdxTreeTrcAdminObjs::onEdtObjectNameFilterEditingFinished);
                }
                break;
            }
            case CModelIdxTreeTrcAdminObjs::EColumnMethodNameFilter: {
                if (pCModelTreeEntry->isLeave()) {
                    QLineEdit* pEdt = new QLineEdit(i_pWdgtParent);
                    pEdt->setObjectName(pCModelTreeEntry->keyInTree() + ".MethodNameFilter");
                    pWdgtEditor = pEdt;
                    QObject::connect(
                        pEdt, &QLineEdit::editingFinished,
                        this, &CDelegateIdxTreeTrcAdminObjs::onEdtMethodNameFilterEditingFinished);
                }
                break;
            }
    ...
    if (mthTracer.areMethodCallsActive(EMethodTraceDetailLevel::ArgsNormal)) {
        QString strMthReturn = pWdgtEditor == nullptr ? "nullptr" : pWdgtEditor->objectName();
...
//------------------------------------------------------------------------------
{
    ...
    QString strMthInArgs;
    if (areMethodCallsActive(m_pTrcAdminObj, EMethodTraceDetailLevel::ArgsNormal)) {
        strMthInArgs = QString(pCmb == nullptr ? "null" : pCmb->objectName()) +
            ", " + QString(pCmb == nullptr ? QString::number(i_idx) : pCmb->currentText());
    }
    CMethodTracer mthTracer(
        /* pTrcAdminObj       */ m_pTrcAdminObj,
        /* eFilterDetailLevel */ EMethodTraceDetailLevel::EnterLeave,
...
//------------------------------------------------------------------------------
void CDelegateIdxTreeTrcAdminObjs::onEdtObjectNameFilterEditingFinished()
//------------------------------------------------------------------------------
{
    QLineEdit* pEdt = dynamic_cast<QLineEdit*>(sender());
    QString strMthInArgs;
    if (areMethodCallsActive(m_pTrcAdminObj, EMethodTraceDetailLevel::ArgsNormal)) {
        strMthInArgs = QString(pEdt == nullptr ? "null" : pEdt->objectName()) +
            ", " + QString(pEdt == nullptr ? "" : pEdt->text());
    }
    CMethodTracer mthTracer(
        /* pTrcAdminObj       */ m_pTrcAdminObj,
        /* eFilterDetailLevel */ EMethodTraceDetailLevel::EnterLeave,
        /* strMethod          */ "onEdtObjectNameFilterEditingFinished",
        /* strMethodInArgs    */ strMthInArgs );
}

//------------------------------------------------------------------------------
void CDelegateIdxTreeTrcAdminObjs::onEdtMethodNameFilterEditingFinished()
//------------------------------------------------------------------------------
{
    QLineEdit* pEdt = dynamic_cast<QLineEdit*>(sender());
    QString strMthInArgs;
    ...
    CMethodTracer mthTracer(
        /* pTrcAdminObj       */ m_pTrcAdminObj,
        /* eFilterDetailLevel */ EMethodTraceDetailLevel::EnterLeave,
        /* strMethod          */ "onEdtMethodNameFilterEditingFinished",
        /* strMethodInArgs    */ strMthInArgs );
}
